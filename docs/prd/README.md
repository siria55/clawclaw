...
- 收到 digest 后，用户可直接回复数字（如 `3`）获取当天对应新闻的原文链接
...
日报条目编号现在按整份 digest 全局连续编号，便于飞书侧用“回复数字”的方式索取对应原文链接。
...
状态页不再只展示连接灯和日志，还会集中展示：

- 飞书运行摘要：配置来源、Webhook、App ID、Chat ID、签名相关配置是否已启用
