...
国内候选搜索会显式收敛到中国语境：查询文本优先使用“中国…”而不是“国内…”，并在 Brave 请求上附带 `country=CN` 和 `search_lang=zh-hans`。
日报条目编号现在按整份 digest 全局连续编号，便于飞书侧用“回复数字”的方式索取对应原文链接；命中后直接返回单独新闻 URL。
Brave 搜索相关配置推荐统一在 WebUI `自动化 > 搜索` 中调整，可同时维护搜索主题与 `Brave Search API Key`，并持久化到 `data/skills/daily-digest/config.json`。
...
| `GET/POST /api/config/daily-digest` | DailyDigest 搜索主题与 Brave Search API Key 配置 |
...
- `自动化 > 搜索` 集中管理 `Brave Search API Key` 与 `daily-digest` 搜索主题，统一写入 `data/skills/daily-digest/config.json`
