...
3. 候选阶段会先过滤百家号等自媒体 / 聚合号链接，再按国内 / 国际两路调用专用 LLM 抽取提示词，将候选链接筛成带 `category` 的结构化文章数组，并尽量补出结构化发布日期 `date`
...
候选新闻搜索默认限制在过去 24 小时内，减少旧闻混入当天日报。
...
Brave Search API Key 可在 WebUI `系统 > 设置` 中配置并持久化到 `data/skills/daily-digest/config.json`；未配置时运行时回退到环境变量 `BRAVE_SEARCH_API_KEY`。
