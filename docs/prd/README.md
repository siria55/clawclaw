...
3. 候选阶段会先过滤百家号等自媒体 / 聚合号链接；其中“国内”候选会在进入 LLM 前先分成“中国大陆来源优先池”和“非大陆回退池”，先抽取大陆媒体 / 机构 / 官网来源，不足时才用港澳台、海外华文或其他境外来源补位；随后按国内 / 国际两路调用专用 LLM 抽取提示词，优先筛出教育、教育科技、AI 教育、教育公司内容，同时保留与教育场景强相关的科技动态，形成带 `category` 的结构化文章数组，并尽量补出结构化发布日期 `date`
...
即便 Brave 返回的“国内”候选里混入港媒、台媒、海外华文或日文站点，产品层也会把它们放入非大陆回退池，不再直接主导“国内资讯”。
...
`日报记录` 里的 run 详情现在会补充展示 LLM 抽取阶段的候选明细、通过率、候选与最终入选对照，以及诊断提示；对于国内抽取，还会额外显示大陆候选数、回退候选数和“大陆优先 / 非大陆回退”阶段，便于定位“为什么国内没出中国大陆新闻”。
