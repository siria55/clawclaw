...
3. 候选阶段会先过滤百家号等自媒体 / 聚合号链接；其中“国内”候选会在进入 LLM 前先按显式白名单分成“中国大陆来源优先池”和“非大陆回退池”，不再把泛 `.cn` / `.com.cn` 一律视作大陆来源；先抽取明确识别出的大陆媒体、政府 / 高校域名，不足时才用港澳台、海外华文或其他境外来源补位；随后按国内 / 国际两路调用专用 LLM 抽取提示词，优先筛出教育、教育科技、AI 教育、教育公司内容，同时保留与教育场景强相关的科技动态，形成带 `category` 的结构化文章数组，并尽量补出结构化发布日期 `date`
4. 同一篇链接如果同时命中国内和国际搜索，去重后会优先保留 `domestic` 提示，避免中国语境命中的候选在合并阶段被错误打成国际
5. 若标题 / 摘要 / 来源里出现繁体中文，会在最终选稿前统一转为简体中文；若出现中文 / 英文之外的其他语言，则统一翻译成简体中文
6. 若 LLM 返回 fenced json 或 near-JSON（如标题引号未转义），解析层会做兜底修复
7. 依据配额裁成国内 10 篇、国际 5 篇，共 15 篇
...
候选新闻搜索默认限制在过去 3 天内，兼顾时效性和中国大陆候选覆盖面。
默认 Brave `news/search` 参数为 `count=20`、`offset=0`、`freshness=p3d`、`safesearch=strict`、`spellcheck=0`；其中 `p3d` 会在运行时自动展开为滚动 3 天日期区间；国内候选搜索还会显式收敛到中国语境：查询文本优先使用“中国…”而不是“国内…”，并在 Brave 请求上附带 `country=CN` 和 `search_lang=zh-hans`。
运行时还会为每条国内 query 额外派生一条带大陆主流媒体提示词的搜索请求，用于补捞中国大陆媒体候选，但最终是否入选仍取决于相关性与时效性。
即便 Brave 返回的“国内”候选里混入港媒、台媒、海外华文、日文站点或伪大陆 `.cn` 域名，产品层也会把它们放入非大陆回退池，不再直接主导“国内资讯”。
`sputniknews.cn`、`archdaily.cn`、`k.sina.cn` 这类站点不会再被当成中国大陆来源抬进“大陆优先”阶段。
...
`日报记录` 里的 run 详情现在会补充展示 LLM 抽取阶段的候选明细、通过率、候选与最终入选对照，以及诊断提示；对于国内抽取，还会额外显示大陆候选数、回退候选数和“大陆优先 / 非大陆回退”阶段，且前端标签与后端共用同一套来源识别逻辑，便于定位“为什么国内没出中国大陆新闻”。
默认 query 已进一步收紧到“中国大陆教育 / 教育科技 / AI 教育 / 教育公司”语境，并只保留少量明确国际 query，避免继续使用过宽的中性词把“国内资讯”扩散到境外来源。
...
- `日报记录` 会读取 `data/skills/daily-digest/runs/*.json`，直接展示 Brave 请求参数、原始返回、LLM 抽取结果与最终入选数；国内抽取还会展示大陆候选与回退候选分层，方便排查“为什么只搜到很少的新闻”或“为什么国内没出大陆新闻”
