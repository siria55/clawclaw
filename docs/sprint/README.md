...
- `daily-digest` 国际区块只保留简体中文 / 英文内容
- 明显为日文、韩文等其他语言的国际文章会在最终入选阶段被拦截
- 抽取提示词与最终筛选规则同步收紧

## 第四十五阶段：国际结果去繁体（Sprint 87）

- `daily-digest` 国际区块进一步收紧为只保留简体中文 / 英文
- 明显繁体中文内容会在最终入选阶段直接拦截
- 港台繁体中文站点会额外做 hostname 识别，降低漏网概率

## 第四十六阶段：日报执行记录可视化（Sprint 88）

- `daily-digest` 每次执行都会把 Brave 请求参数、原始返回和关键筛选阶段数据落盘
...
## 第四十九阶段：候选去重保留国内提示（Sprint 94）

- 同一篇链接如果同时命中国内和国际搜索，去重后会优先保留国内提示
- 这样中国语境命中的候选不会在合并阶段被错误洗到国际池
- 重跑验证后，国内大陆候选数已从 0 回升到 1

## 第五十阶段：默认 query 收紧到中国大陆教育语境（Sprint 95）

- `daily-digest` 默认 query 进一步收紧到“中国大陆教育 / 教育科技 / AI 教育 / 教育公司”语境
...
## 第五十二阶段：大陆来源识别收紧（Sprint 97）

- `daily-digest` 的大陆来源识别改为显式白名单，不再把泛 `.cn` / `.com.cn` 当成中国大陆来源
- `sputniknews.cn`、`archdaily.cn`、`k.sina.cn` 这类伪大陆域名会进入非大陆回退池，不再挤占大陆优先候选
- `日报记录` 页改为和后端共用同一套来源识别逻辑，`大陆来源` 标签与真实筛选口径保持一致

---

## 当前落点（Sprint 97）

**框架**：ClawServer + WebServer 24/7 常驻，支持飞书 / 企业微信，LLM 支持 Anthropic Claude 和 OpenAI ChatGPT，运行态数据统一存 `data/`。

**WebUI**：6 个一级 tab（`对话` / `内容` / `日报记录` / `自动化` / `IM` / `系统`），各自下设二级 tab，旧 hash 向后兼容。

**DailyDigestSkill**：
- 候选来源切换到 Brave Search API，默认搜索滚动过去 3 天，国内搜索带 `country=CN` / `search_lang=zh-hans`
- 国内候选按显式白名单分”大陆优先 / 非大陆回退”两池，`.cn` 泛域名不再直接当大陆来源
- LLM 筛选优先教育 / 教育科技 / AI 教育 / 教育公司内容；繁体转简体，其他语言译简体
- 条目编号全局连续，飞书里回复数字可取对应原文链接
- 生成与发送拆成两个 Cron Job，`daily-digest-send` 只发当天 PNG，缺失时发失败提醒
- 每次执行落盘 `runs/{runId}.json`，`日报记录` 可查看各筛选阶段明细（含大陆 / 非大陆候选数）

**飞书**：群聊仅响应 `@机器人`；支持 Markdown 渲染；”给我今天的新闻”直接返回图片；各处 `ou_` / `oc_` ID 均显示解析后的用户名 / 群名。
