...
- WebUI 一级导航为 6 个大 tab：`对话` / `内容` / `日报记录` / `自动化` / `IM` / `系统`
...
- `内容` 内部为 `新闻库` / `记忆库`
- `日报记录` 可直接查看 `daily-digest` 每次执行的 Brave 请求参数、原始返回和筛选结果
...
- `daily-digest` 每次执行现在都会额外落一份 `runs/{runId}.json`，便于排查“为什么只搜到很少的新闻”
...
- `daily-digest` 的大陆来源识别现在只接受明确白名单域名，不再把伪大陆 `.cn` 站点误抬到大陆优先池
