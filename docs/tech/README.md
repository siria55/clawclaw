...
WebUI 手动运行：
onRunSkill → skill.run(ctx) → SkillResult（outputPath 忽略，仅展示日志）
```
...
9. CronScheduler 收到 outputPath，调用 `feishu.sendImage(chatId, outputPath)`
...
**SkillsView 日志面板：** 手动触发 Skill 后，通过 `fetch` + `ReadableStream` 读取 SSE 流，实时渲染深色终端风格日志，自动滚动到底部。
