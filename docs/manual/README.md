...
- `daily-digest` 卡片会把搜索主题配置、手动运行、日志和图片预览放在一起；保存后可直接按新配置运行
...
3. 候选阶段会先过滤百家号等自媒体 / 聚合号链接，再按国内 / 国际两路用专用 LLM 抽取提示词，把候选链接筛成带 `category` 的文章列表（JSON）
...
当前 Brave 检索默认只搜索过去 24 小时内的新闻。
新闻条目现在会显示“来源 + 新闻时间”；若某条搜索结果没稳定提取到时间，则该条只显示来源。
最终结果会尽量优先使用主流媒体、公司官网和权威发布；百家号、搜狐号、网易号、企鹅号、头条号等自媒体来源不会进入日报。
导出的 JSON 现在会额外带上结构化发布日期 `date`；若搜索结果里只有相对时间，系统会按当天日报日期做最佳努力换算。
PNG 截图当前使用 4x 高清输出，版面宽度不变，但图片更适合放大查看。
搜索主题可在 `自动化 > Skills > daily-digest` 卡片内直接修改；保存到 `data/skills/daily-digest/config.json` 后，下一次运行 skill 时自动生效。
同一张卡片里也可以直接手动运行、看日志和看最新截图预览。
`系统 > 设置` 里可以直接保存 `Brave Search API Key`；该配置同样写入 `data/skills/daily-digest/config.json`。
