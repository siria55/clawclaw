...
运行 `pnpm dev:web` 后访问 `http://localhost:5173`，界面包含八个标签页，每个标签页对应独立 URL，支持直接访问和浏览器前进/后退。
...
查看 IM 相关信息，页内分为三个子 tab：
- `状态` 子 tab：页内导航固定在页面右侧，可跳到平台连接、飞书运行、群聊摘要
- `状态` 子 tab：展示 IM 平台连接状态、飞书运行摘要、飞书群聊列表
- `消息` 子 tab：展示 IM 事件日志实时轮询，并支持按群聊 / 直发筛选
- `消息` 子 tab：展示平台、群名、用户 ID、会话 ID、回复摘要
- `配置` 子 tab：展示飞书 IM 配置表单和当前飞书运行摘要
- `配置` 子 tab：配置保存在 `data/im/im-config.json`，保存后立即生效，无需重启
- `配置` 子 tab：若希望 Agent 能读取部门人数、直属成员等组织信息，还需在飞书开放平台开通通讯录 / 部门读取权限
- `配置` 子 tab：Agent 回复若包含明显 Markdown 结构，飞书会尽量按原生 Markdown 渲染发送
- `配置` 子 tab：如果需要强制直发 Markdown，可在 `Cron` 里把直发类型切到 `Markdown`
- `配置` 子 tab：在飞书里直接问“给我今天的新闻”时，系统会优先发送今日新闻图片；文字版可回复“今天新闻文本版”
- `配置` 子 tab：在飞书群聊里带上 `@机器人` 再发这两类指令，也同样生效
- `配置` 子 tab：如果今日日报文件还不存在，服务端会先自动执行一次 `daily-digest` 再发送
- 点击刷新实时更新
...
- 页内导航：固定在页面右侧，可直接跳到 Agent、飞书文档、DailyDigest、模型
...
- 日报条目编号按整份 digest 全局连续编号
- 在飞书里收到 digest 后，可直接回复数字，例如 `3`，机器人会返回对应新闻的单独原文链接
- 在飞书群聊里即使写成 `@机器人 3`，也会按数字回复处理
