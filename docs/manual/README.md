...
- `搜索`（`/#search`）：集中管理 Brave 搜索相关配置，可直接修改 `Brave Search API Key` 和 `daily-digest` 搜索主题；保存后写入 `data/skills/daily-digest/config.json`
...
Brave 搜索相关配置推荐在 `自动化 > 搜索` 统一维护；这里可以同时修改 `Brave Search API Key` 和搜索主题，并持久化到 `data/skills/daily-digest/config.json`。
