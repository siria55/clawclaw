...
4. 再按国内 / 国际两路用专用 LLM 抽取提示词，优先保留教育、教育科技、AI 教育、教育公司内容，同时保留与教育场景强相关的科技动态，把候选链接筛成带 `category` 的文章列表（JSON）
5. 若模型返回 fenced json 或格式略脏的 near-JSON，系统会自动做兼容解析
6. 按国内 10 篇、国际 5 篇裁成最终日报；国内大陆来源会有额外排序优先级，减少回退来源压过大陆新闻
7. 将文字内容填入 HTML 模板，截图为 PNG，同时保存 `.html / .md / .png / .json` 四个文件
8. 返回 PNG 路径；WebUI 自动展示预览，Cron `sendSkillOutput` 发送到飞书
9. 同时把本次执行的 Brave 请求参数、返回结果、候选数、抽取结果和最终入选内容持久化保存到 `data/skills/daily-digest/runs/{runId}.json`
