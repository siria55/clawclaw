...
- `LLM 抽取` 区块会额外显示送入 LLM 的候选明细、抽取通过率、候选与最终入选对照，以及诊断提示；国内链路还会显示“大陆优先 / 非大陆回退”阶段和对应候选数；`大陆来源` 标签现在和后端共用同一套规则，不再把泛 `.cn` 误标成大陆
...
- `Cron` 支持绑定 Skill（`skillId`）或发送 Skill 图片（`sendSkillOutput`）；其中 `daily-digest` 只发送当天图片
...
- 默认搜索主题现在更明确落在中国大陆教育语境，例如 `中国 教育部 AI 教育`、`中国 智慧教育`、`中国 高校 AI 教育`、`中国 教育科技 公司`，再搭配少量国际主题如 `OpenAI education`
...
3. 候选阶段会先过滤百家号等自媒体 / 聚合号链接；其中“国内”候选会先按显式白名单分成中国大陆来源优先池和非大陆回退池，不再把泛 `.cn` / `.com.cn` 一律视作大陆来源；先抽取明确识别出的大陆媒体、政府 / 高校域名，不足时才用港澳台、海外华文或其他境外来源补位
4. 如果同一篇链接同时命中国内和国际搜索，系统会优先保留国内提示，不再在去重阶段被错误改成国际
5. 再按国内 / 国际两路用专用 LLM 抽取提示词，优先保留教育、教育科技、AI 教育、教育公司内容，同时保留与教育场景强相关的科技动态，把候选链接筛成带 `category` 的文章列表（JSON）
6. 若模型返回 fenced json 或格式略脏的 near-JSON，系统会自动做兼容解析
7. 按国内 10 篇、国际 5 篇裁成最终日报；国内大陆来源会有额外排序优先级，减少回退来源压过大陆新闻
8. 将文字内容填入 HTML 模板，截图为 PNG，同时保存 `.html / .md / .png / .json` 四个文件
9. 返回 PNG 路径；WebUI 自动展示预览，Cron `sendSkillOutput` 发送到飞书
10. 同时把本次执行的 Brave 请求参数、返回结果、候选数、抽取结果和最终入选内容持久化保存到 `data/skills/daily-digest/runs/{runId}.json`
...
当前 Brave 检索默认只搜索过去 3 天内的新闻。
默认请求参数会带 `count=20`、`offset=0`、`freshness=p3d`、`safesearch=strict`、`spellcheck=0`；其中 `p3d` 会在运行时自动展开为滚动 3 天日期区间，例如 2026-03-31 会展开为 `2026-03-29to2026-03-31`；国内搜索默认还会附带 `country=CN` 和 `search_lang=zh-hans`。
每条国内 query 运行时还会额外派生一条带大陆主流媒体提示词的搜索请求，用来补捞大陆媒体候选。
即便 Brave 的“国内”结果混入港媒、台媒、海外华文、日文站点或伪大陆 `.cn` 域名，系统也会先把中国大陆来源单独抽取，不再让这些来源直接主导“国内资讯”。
`sputniknews.cn`、`archdaily.cn`、`k.sina.cn` 这类站点不会再被标成中国大陆来源。
...
- **Skill 投递**（`sendSkillOutput`）：发送指定 Skill 的 PNG 到飞书；`daily-digest` 只发送当天 PNG
...
- 10:00 `sendSkillOutput: "daily-digest"` — 发送当日日报到飞书；若当天缺失则发送失败提醒
