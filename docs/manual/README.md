...
2. 用一次专用 LLM 抽取提示词，把候选链接筛成文章列表（JSON）
3. 若模型返回 fenced json 或格式略脏的 near-JSON，系统会自动做兼容解析
4. 渲染成 HTML 日报，截图为 PNG，同时保存 `.html / .md / .png / .json` 四个文件
5. 返回 PNG 路径；WebUI 自动展示预览，Cron `sendSkillOutput` 发送到飞书

日报 HTML 的视觉样式来自 `src/skills/daily-digest/layout.css`；Skill 每次运行都会读取这份样式并写入当天导出的 HTML。
