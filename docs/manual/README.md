...
- 日志内容包括：浏览器访问 URL、文章获取进度、文件保存路径等
...
每次运行：
1. 用 Playwright 浏览器 + 子 Agent（LLM 驱动）搜索百度新闻
2. 提取文章列表（JSON），渲染成 HTML 日报
3. 截图为 PNG，同时保存 `.html / .md / .png / .json` 四个文件
4. 返回 PNG 路径，由 CronScheduler 自动发送到飞书群

输出目录：`data/skills/daily-digest/`

### Cron 绑定 Skill

在设置页新增 Cron 任务时，填入 `skillId: "daily-digest"`，即可定时执行并自动推送截图到飞书。
...
- **Skill 模式**（`skillId`）：执行指定 Skill，生成的截图自动发送到飞书
