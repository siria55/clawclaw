...
运行 `pnpm dev:web` 后访问 `http://localhost:5173`，界面包含九个标签页，每个标签页对应独立 URL，支持直接访问和浏览器前进/后退。
...
查看 IM 平台和飞书群聊状态：
- 页内导航：固定在页面右侧，可直接跳到平台连接、飞书运行、群聊摘要
- IM 平台连接状态（飞书 / 企业微信）
...
- 当机器人被加入飞书群时，系统会记录群 ID 和群名，`IM 状态` 页可直接查看已记录群聊
