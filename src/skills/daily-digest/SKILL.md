...
4. 跨关键词去重后，先过滤百家号等自媒体 / 黑名单链接，再按国内 / 国际分别调用 LLM 优先筛选教育、教育科技、AI 教育、教育公司，以及与教育场景强相关的科技动态，并结构化为 JSON（含 `category`）；国际结果只保留中文 / 英文内容，日文、韩文等其他语言会在最终入选阶段被拦截；新闻时间继续沿用 Brave 返回的时间 hint，并最佳努力推导结构化 `date`
5. 按国内 10 / 国际 5 的配额裁剪
